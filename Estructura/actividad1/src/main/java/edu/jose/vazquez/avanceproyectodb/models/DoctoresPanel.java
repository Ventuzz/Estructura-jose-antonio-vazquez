...
public class DoctoresPanel extends JPanel {
    ...
    public DoctoresPanel() {
        ...
        actions.add(btnCrear); actions.add(btnGuardar);
        ...
        form.setAlignmentX(LEFT_ALIGNMENT); actions.setAlignmentX(LEFT_ALIGNMENT); search.setAlignmentX(LEFT_ALIGNMENT);
        left.add(form); left.add(Box.createVerticalStrut(8)); left.add(actions); left.add(Box.createVerticalStrut(8)); left.add(search);
        ...
        split.setDividerLocation(440); split.setResizeWeight(0.0); split.setOneTouchExpandable(true);
    ...
    private void registrar() {
        try {
            ...
            int id = service.crear(d);
            cargarTabla(null);
            JOptionPane.showMessageDialog(this, "Doctor registrado. ID: "+id+" (auditoría registrada)");
            tfNombre.setText(""); tfCorreo.setText(""); tfTelefono.setText("");
    ...
    private void guardarCambios() {
        try {
            ...
            JOptionPane.showMessageDialog(this, "Doctor actualizado (auditoría registrada).");
        } catch (IllegalArgumentException ex) {
            JOptionPane.showMessageDialog(this, ex.getMessage(), "Validación", JOptionPane.WARNING_MESSAGE);
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this, ex.toString(), "Error", JOptionPane.ERROR_MESSAGE);
    ...
    private void cargarTabla(String filtro) {
        ...
        for (var d : service.listar(filtro)) {
            model.addRow(new Object[]{ d.id, d.nombre, d.correo, d.telefono });
        }
    }
}
